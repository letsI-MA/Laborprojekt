...
class Fractals:
    ...
    def draw_selection_rectangle(self, screen):
        if self.mouseDown:
            mouse_pos = gametest.mouse.get_pos()
            ...
            gametest.draw.rect(screen, (255, 255, 255), (x, y, width, height), 3)
    ...
    def mouse_up(self):
        if self.mouseDown:
            ...
            if width > self.min_selection_size and height > self.min_selection_size:
                rect = gametest.Rect(left, top, width, height)
                self.zoomRect(rect)
            else:
    ...
    def mouse_move(self, screen):
        self.mouse_pos = gametest.mouse.get_pos()
        self.julia_preview()
    ...
    def redraw(self):
        ...
        pygame_surface = gametest.pixelcopy.make_surface(color_data)
    ...
    def draw_julia(self, n):
        ...
        if n > len(self.juliasets):
            return
        gametest.display.set_caption("Julia")
    ...
    def update(self, screen):
        ...
        if preview_enabled:
            ...
            preview_surface = gametest.pixelcopy.make_surface(julia_preview_color_data)
    ...
    def save_screen(self, screen):
        ...
        gametest.image.save(screen, filename)
...
def main():
    ...
    screen = gametest.display.set_mode((width, height))
    gametest.display.set_caption("Mandlebrot")
    clock = gametest.time.Clock()
    ...
    running = True
    while running:
        for event in gametest.event.get():
            if event.type == gametest.QUIT:
                running = False
            elif event.type == gametest.MOUSEBUTTONDOWN:
                if event.button == 1:  # Left mouse button
                    fractals.mouse_down(gametest.mouse.get_pos())
            elif event.type == gametest.MOUSEBUTTONUP:
                if event.button == 1:  # Left mouse button
                    fractals.mouse_up()
            elif event.type == gametest.KEYDOWN:
                if event.key == gametest.K_KP_PLUS:
                    fractals.ZoomIn()
                    fractals.redraw()
                elif event.key == gametest.K_KP_MINUS:
                    fractals.ZoomOut()
                    fractals.redraw()
                elif event.key == gametest.K_s:
                    fractals.save_screen(screen)
                elif event.key == gametest.K_j:
                    fractals.set_mode(DrawingMode.JULIA)
                    gametest.display.set_caption("Julia")
                    fractals.redraw()
                elif event.key == gametest.K_m:
                    fractals.set_mode(DrawingMode.MANDELBROT)
                    gametest.display.set_caption("Mandlebrot")
                    fractals.redraw()
                elif event.key == gametest.K_p:
                    preview_enabled = not preview_enabled
                    #fractals.redraw()
                elif event.key == gametest.K_1:
                    fractals.draw_julia(0)
                elif event.key == gametest.K_2:
                    fractals.draw_julia(1)
                elif event.key == gametest.K_3:
                    fractals.draw_julia(2)
                elif event.key == gametest.K_4:
                    fractals.draw_julia(3)
                elif event.key == gametest.K_5:
                    fractals.draw_julia(4)
                elif event.key == gametest.K_6:
                    fractals.draw_julia(5)
                elif event.key == gametest.K_7:
                    fractals.draw_julia(6)
                elif event.key == gametest.K_8:
                    fractals.draw_julia(7)
                elif event.key == gametest.K_9:
                    fractals.draw_julia(8)
                elif event.key == gametest.K_0:
                    fractals.draw_julia(9)
                elif event.key == gametest.K_q:
                    fractals.draw_julia(10)
                elif event.key == gametest.K_w:
                    fractals.draw_julia(11)
                elif event.key == gametest.K_e:
                    fractals.draw_julia(12)
                elif event.key == gametest.K_r:
